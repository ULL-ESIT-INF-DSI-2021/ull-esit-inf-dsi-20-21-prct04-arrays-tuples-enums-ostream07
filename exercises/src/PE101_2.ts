...
function complexProduct(arrayComplex1, arrayComplex2) {
  ...
  let scarlarResult: number = 0;
  for (let i: number = 0; i < 2; i++) {
    arrayResult[i] = arrayComplex1[i] * arrayComplex2[i];
    if ( i === 1) {
      arrayResult[i] = - arrayResult[i];
      for (let j: number = 0; j < arrayResult.length; j++) {
...
function complexNumber(arrayComplex1, arrayComplex2) {
  const realValue1: number = 1;
  const imaginaryValue1: number = 7;
  const realValue2: number = 2;
  const imaginaryValue2: number = 4;
